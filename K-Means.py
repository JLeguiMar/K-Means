...
Original file is located at
    https://colab.research.google.com/drive/1VsT795GpgA0YiEXALsS0GbKGd846s6OX
...
#Aprendizaje no Supervisado CLUSTERING con los datos sobre Flores de Iris
import numpy as np
import pandas as pd
...
plt.subplot(1,2,1)
plt.scatter(x[:,0],x[:,1],c=colormap[y[:50]])
plt.scatter(x[:,0],x[:,2],c=colormap[y[:50]])
...
x=iris.data
y=iris.target
# Create an instance of KMeans with the desired number of clusters (e.g., 3)
...
colormap=np.array(['red','lime','black'])
plt.subplot(1,2,1)
# Use the entire y array to specify colors for all 150 points
plt.scatter(x[:,0],x[:,1],c=colormap[y])
# Use the entire y array to specify colors for all 150 points
plt.scatter(x[:,0],x[:,2],c=colormap[y])
plt.title('Clusters de Iris Dataset con k-Means')
plt.xlabel('Longitud de Sépalo')
plt.ylabel('Longitud de Pétalo')
plt.show()
...
# Visualización 1: Centros de Clusters
plt.figure(figsize=(14, 7))
colormap = np.array(['red', 'lime', 'black'])

# Scatterplot de las dimensiones originales con centros de clusters
plt.subplot(1, 2, 1)
plt.scatter(x[:, 0], x[:, 1], c=colormap[y_kmeans], s=50, label="Clusters")
plt.scatter(kmeans.cluster_centers_[:, 0], kmeans.cluster_centers_[:, 1],
            s=200, c='yellow', marker='X', edgecolor='k', label='Centroides')
plt.title('K-Means Clustering (Centros de Clusters)')
plt.xlabel('Longitud de Sépalo')
plt.ylabel('Ancho de Sépalo')
plt.legend()

# Visualización 2: Reducción de Dimensionalidad con PCA
pca = PCA(2)
x_pca = pca.fit_transform(x)
plt.subplot(1, 2, 2)
plt.scatter(x_pca[:, 0], x_pca[:, 1], c=colormap[y_kmeans], s=50)
...
# Matriz de confusión entre los clusters y las etiquetas reales
cm = confusion_matrix(y, y_kmeans)
print("Matriz de Confusión entre Clusters y Etiquetas Reales:")
print(cm)

# Calcular el índice de silueta para evaluar los clusters
silhouette_avg = silhouette_score(x, y_kmeans)
print(f"Índice de Silueta Promedio: {silhouette_avg:.2f}")
...
archivo_csv = 'crop_production.csv'

data = pd.read_csv(archivo_csv)

# Eliminar 'FarmID'
data = data.drop('FarmID', axis=1)


#Separar las variables independientes (x) y la dependiente (y)

data = data.drop('CropYield', axis=1)
x = data.drop('FertilizerUsed', axis = 1)
#Supón que 'target' es la columna de etiqueta/clase
y = data['FertilizerUsed']
...
# Convertir 'CropYield' a binario (rendimiento alto/bajo)
data['CropYield'] = data['CropYield'].apply(lambda x: 1 if x < 3000 else 0)


# Convertir la columna categórica 'SoilQuality' en variables dummy
#data = pd.get_dummies(data, columns=['SoilQuality'], drop_first=True)
categorical_columns = ['SoilQuality', 'Rainfall', 'FertilizerUsed']
...
from sklearn.metrics import accuracy_score, confusion_matrix#Permite calcular la precisión del modelo de clasificación
import pandas as pd # permite la manipuacion y analisis de los datos
import matplotlib.pyplot as plt
from sklearn.metrics import accuracy_score, confusion_matrix, classification_report#Permite calcular la precisión del modelo de clasificación # Added classification_report to the import statement
import seaborn as sns # Permite crear gráficos estadísticos
...
plt.title('Crop production')#Crea el titulo de la grafica
...
# Fix: Remove header=None and use the actual header row
pima = pd.read_csv('crop_production.csv')  # This assumes your CSV has a header row
pima.head()

# Ensure 'FertilizerUsed' and 'Rainfall' are numeric
pima['FertilizerUsed'] = pd.to_numeric(pima['FertilizerUsed'], errors='coerce')
pima['Rainfall'] = pd.to_numeric(pima['Rainfall'], errors='coerce')

feature_cols = ['FertilizerUsed', 'Rainfall']
x = pima[feature_cols]
y = pima.CropYield
...
clf = clf.fit(x_train, y_train)  # Fit the model using training data
y_pred = clf.predict(x_test)     # Predict using the test data

print("Accuracy:", accuracy_score(y_test, y_pred))

import pandas as pd  #permite manipulación de datos
import numpy as np  #se importa la biblioteca numpy
import matplotlib.pyplot as plt # importa pyplot para poder crear graficos
from sklearn.tree import DecisionTreeClassifier, plot_tree #importa DecisionTreeClassifier para poder crear el modelo de árboñ
# e importa Plot tree
from sklearn.model_selection import train_test_split# permite dividir los datos en los conjuntos de entrenamiento y prueba
from sklearn.metrics import classification_report, accuracy_score  #implementa las funciones que permiten evaluar el modelo
from tensorflow.keras.datasets import mnist #MNIST Instituto Nacional de Normas y Tecnología Modificado permite importar los datos de Minist
#desde la biblioteca de tensorflow.keras.datasets

# Cargar el conjunto de datos MNIST
(x_train, y_train), (x_test, y_test) = mnist.load_data()

# Preprocesamiento de los datos para el árbol de decisión
x_train_flat = x_train.reshape(x_train.shape[0], -1)
x_test_flat = x_test.reshape(x_test.shape[0], -1)

# Dividir el conjunto de datos
x_train_split, x_val_split, y_train_split, y_val_split = train_test_split(x_train_flat, y_train, test_size=0.3, random_state=42)

# Crear y entrenar el árbol de decisión con max_depth ajustado
clf = DecisionTreeClassifier(max_depth=5)  # Ajusta max_depth según sea necesario
...
# Mostrar una imagen del conjunto de datos MNIST y su predicción
plt.imshow(x_test[0], cmap='gray')
plt.title(f"Etiqueta real: {y_test[0]}, Predicción: {y_pred[0]}")
plt.show()
...
for i, error in enumerate(errors[:9]):
    plt.subplot(3, 3, i+1)
    plt.imshow(x_test[error], cmap='gray')
    plt.title(f"Pred: {y_pred[error]}, True: {y_test[error]}")
plt.show()
...
# Preprocesamiento de los datos para el árbol de decisión
# Aplanar las imágenes de 28x28 a 784 características
x_train_flat = x_train.reshape(x_train.shape[0], -1)
x_test_flat = x_test.reshape(x_test.shape[0], -1)
...
# Dividir el conjunto de datos (por si se desea hacer split adicional)
x_train_split, x_val_split, y_train_split, y_val_split = train_test_split(x_train_flat, y_train, test_size=0.3, random_state=42)

# Crear y entrenar el árbol de decisión
clf = DecisionTreeClassifier(max_depth=4) #ajusta los datos que se muestran del arbol
clf = clf.fit(x_train_split, y_train_split)
